<section id="section-3" class="feature-section bg-swoosh_solid bg-cover no-pad text-center">

  <h2 class="color-brand-dark-blue fw-bold">Our Partners</h2>
	
  <div class="container-fluid px-0">
    <div class="overflow-auto">
      <div class="d-flex flex-nowrap" style="gap: 2.1rem; padding: 1rem 3.8rem; scroll-snap-type: x mandatory;">

        <!-- Card 1 -->
        <div class="card">
          <div class="bg-volunteer_food bg-cover">&nbsp;</div>
          <div class="card-body text-start">
            <h5 class="card-title fw-bold">Non-Profits</h5>
            <p class="card-text color-brand-dark-blue">Food banks, relief/emergency programs, feeding programs</p>
            <a href="#" class="btn btn-primary">More <i class="fas fa-arrow-right"></i></a>
          </div>
        </div>

        <!-- Card 2 -->
        <div class="card">
          <div class="bg-freepik_adjust bg-cover">&nbsp;</div>
          <div class="card-body text-start">
            <h5 class="card-title fw-bold">Price Impact Stores</h5>
            <p class="card-text color-brand-dark-blue">Food banks, relief/emergency programs, feeding programs</p>
            <a href="#" class="btn btn-primary">More <i class="fas fa-arrow-right"></i></a>
          </div>
        </div>

        <!-- Card 3 -->
        <div class="card">
          <div class="bg-couple_supermarket bg-cover">&nbsp;</div>
          <div class="card-body text-start">
            <h5 class="card-title fw-bold">Alternative Retail</h5>
            <p class="card-text color-brand-dark-blue">Food banks, relief/emergency programs, feeding programs</p>
            <a href="#" class="btn btn-primary">More <i class="fas fa-arrow-right"></i></a>
          </div>
        </div>

        <!-- Card 4 -->
        <div class="card">
          <div class="bg-volunteer_food bg-cover">&nbsp;</div>
          <div class="card-body text-start">
            <h5 class="card-title fw-bold">Food Service</h5>
            <p class="card-text color-brand-dark-blue">Food banks, relief/emergency programs, feeding programs</p>
            <a href="#" class="btn btn-primary">More <i class="fas fa-arrow-right"></i></a>
          </div>
        </div>

        <!-- Ghost Spacer -->
        <div class="scroll-pad-end"></div>


      </div>
    </div>

    <a href="#" class="btn btn-primary bg-brand-dark-blue color-white" style="margin: 25px 30px 30px">Become A Partner <i class="fas fa-arrow-right"></i></a>
  </div>
</section>